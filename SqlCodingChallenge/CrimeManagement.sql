...
WITH suspect_crime_types AS (
    SELECT 
        s.name,
        COUNT(DISTINCT CASE WHEN c.incidentType = 'Homicide' THEN 1 END) AS homicide_count,
        COUNT(DISTINCT CASE WHEN c.incidentType NOT IN ('Robbery', 'Homicide') THEN 1 END) AS other_cases
    FROM suspects s
    JOIN crime c ON s.crimeID = c.crimeID
    GROUP BY s.name
),
qualified_suspects AS (
    SELECT name
    FROM suspect_crime_types
    WHERE homicide_count >= 1 AND other_cases = 0
)
SELECT 
    s.suspectID, s.name, s.crimeID, c.incidentType, c.incidentDate, c.location, c.status
    FROM suspects s
    join crime c ON s.crimeID = c.crimeID
    join qualified_suspects q ON s.name = q.name;
